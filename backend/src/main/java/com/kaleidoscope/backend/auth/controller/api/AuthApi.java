...
public interface AuthApi {
    ...
    ResponseEntity<AppResponse<UserRegistrationResponseDTO>> registerUser(
    ...
    ResponseEntity<AppResponse<UserLoginResponseDTO>> authenticateUser(
    ...
    ResponseEntity<AppResponse<String>> logoutUser(
    ...
    ResponseEntity<AppResponse<String>> forgotPassword(
    ...
    ResponseEntity<AppResponse<String>> resetPassword(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Invalid old password or new password format")
    })
    ResponseEntity<AppResponse<String>> changePassword(
    ...
    ResponseEntity<AppResponse<UserLoginResponseDTO>> renewToken(
    ...
    ResponseEntity<AppResponse<String>> sendVerificationEmail(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Email verification request", required = true,
                    content = @Content(schema = @Schema(implementation = VerifyEmailRequestDTO.class)))
            @RequestBody VerifyEmailRequestDTO verifyEmailRequestDTO);
    ...
    @Operation(summary = "Generate a one-time SSE ticket",
            description = "Generates a short-lived (30s), one-time ticket for authenticating an SSE connection. This endpoint requires a valid Bearer token.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Ticket generated successfully"),
            @ApiResponse(responseCode = "401", description = "Unauthorized (Invalid Bearer token)")
    })
    @PostMapping
    @PreAuthorize("isAuthenticated()")
    ResponseEntity<AppResponse<SseTicketResponseDTO>> generateSseTicket();
}
